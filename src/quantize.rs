...
pub fn encode(width: usize, height: usize, input: &[f32]) -> Vec<i32> {
    ...
    ZigZagIterator::new(width, height)
        .map(|(x, y)| {
...
pub fn decode(width: usize, height: usize, input: &[i32]) -> Vec<f32> {
    ...
    // the first block of zeros is omitted. we'll use the difference between
    // input.len() and width*height to tell how many
...
fn get_quantize_coeff(width: usize, height: usize, x: usize, y: usize) -> f32 {
    ((width * height) / 10000 + (x + y) * 20) as f32
}
...
impl ZigZagIterator {
    fn new(width: usize, height: usize) -> ZigZagIterator {
        ...
        ZigZagIterator {
            width: width,
            height: height,
            previous_result: if (width + height) % 2 == 0 {
                (width - 1, height)
            } else {
                (width, height - 1)
            },
    ...
    fn next(&mut self) -> Option<(usize, usize)> {
        if self.previous_result == (0, 0) {
            ...
            let next = if diagonal_index % 2 == 1 {
                ...
                // first check if the x is already at 0. if it is,
                // we can't go left any further, so go up instead
                if self.previous_result.0 == 0 {
...
mod test {
    ...
    fn test_zigzag() {
        ...
        // the 2 numbers in each tuple are the width and height, and the vec
        // contains the expected order the iterator will visit elements in (it's
        // a flattened 2d array in row major order)
        let input_list = vec![
            (1,1,vec![0]),
            (1,5,vec![4,3,2,1,0]),
            (2,2,vec![3, 2,    1, 0]),
